...
| 版本 | versionCode | 内容 |
|---|---|---|
| **2.4** | 24 | **修复 π / e 的有效位数被卡在 202 个字符**：原来 `pi`/`e` 用的是源码里硬编码的 200 位小数常量，所以设置里把有效位数调到 1000 甚至 20000，结果也只有那 202 个字符。现在改成**运行时按需计算** —— π 用 Brent–Salamin(AGM) 迭代（每轮有效位数翻倍，20000 位也只需 ~18 轮）、e 用 Σ1/n! 级数，并带内存缓存。实测 π / e 在 50 / 200 / 1000 / 5000 / 20000 位下都与独立的 Python 实现（Machin 公式）逐位一致。 |
...
* `pi` / `e` 按当前**有效位数**实时计算（不是硬编码常量），设置里调到 1000、10000 位都有效，
  上限就是精度的上限 20000 位
